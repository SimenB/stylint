...
describe('Linter Style Checks: ', function() {
	...
	describe('brackets', function() {
		...
		it ('should return true if illegal bracket found on line (not interpolation, not hash)', function() {
			...
			assert.equal( true, app.brackets('.{interpolated}-class {', false) );
	...
	describe('efficient', function() {
		it('should return false if value is not efficient', function() {
			...
			assert.equal( false, app.efficient( 'margin 0 0' ) );
			assert.equal( false, app.efficient( 'margin: 0 5px 0 5px' ) );
			assert.equal( false, app.efficient( 'margin 5px 0 5px' ) );
			assert.equal( false, app.efficient( 'margin: 5px 0 5px 0' ) );
			assert.equal( false, app.efficient( 'margin 0 5px 0' ) );
			assert.equal( false, app.efficient( 'margin: 0 5px 5px 5px' ) );
			assert.equal( false, app.efficient( 'padding 0 0 0 0' ) );
			assert.equal( false, app.efficient( 'padding: 0 0 0' ) );
			assert.equal( false, app.efficient( 'padding 0 0' ) );
			assert.equal( false, app.efficient( 'padding: 0 5px 0 5px' ) );
			assert.equal( false, app.efficient( 'padding 5px 0 5px' ) );
			assert.equal( false, app.efficient( 'padding: 5px 0 5px 0' ) );
			assert.equal( false, app.efficient( 'padding 0 5px 0' ) );
			assert.equal( false, app.efficient( 'padding: 0 5px 5px 5px' ) );
		});

		it('should return true if value is efficient', function() {
			assert.equal( true, app.efficient( 'margin 0 5px' ) );
			assert.equal( true, app.efficient( 'margin: 5px 0' ) );
			assert.equal( true, app.efficient( 'margin 5px 0 0' ) );
			assert.equal( true, app.efficient( 'margin: 0' ) );
			assert.equal( true, app.efficient( 'margin 5px' ) );
			assert.equal( true, app.efficient( 'padding 0 5px' ) );
			assert.equal( true, app.efficient( 'padding: 5px 0' ) );
			assert.equal( true, app.efficient( 'padding 5px 0 0' ) );
			assert.equal( true, app.efficient( 'padding: 0' ) );
			assert.equal( true, app.efficient( 'padding 5px' ) );
			assert.equal( true, app.efficient( 'padding: 1px 2px 3px 4px' ) );
		});

		it('should return undefined if no margin or padding found', function() {
			assert.equal( undefined, app.efficient( '.border-strong' ) );
		})
	...
	describe('extends style', function() {
		...
		it('should return undefined if no extend on line', function() {
			assert.equal( undefined, app.extend('.classTest', '@extend') );
			assert.equal( undefined, app.extend('.classTest2', '@extends') );
		});
	...
	describe('placeholder style', function() {
		...
		it('should return true if placeholder var is used', function() {
			...
			assert.equal( true, app.placeholder('@extend $placeholderVar') );
		});
	...
	describe('quote style', function() {
		...
		it('should return false if no quotes found', function() {
			assert.equal( false, app.quotes( '$var = #000 ', 'single' ) );
			assert.equal( false, app.quotes( '$var = #000 ', 'double' ) );
		});
	...
	describe('valid property', function() {
		...
		it ('should return true if property is valid', function() {
			...
			assert.equal( true, app.valid( '-webkit-border-radius 0', valid ) );
			assert.equal( true, app.valid( '.el:hover', valid ) );
	...
	describe('var style check', function() {
		...
		it('should return false if $ if block var', function() {
			assert.equal( false, app.varStyle('myVar = @block') );
		});
